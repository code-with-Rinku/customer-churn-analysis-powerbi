...
select PaymentMethod,count(PaymentMethod)as count_PaymentMethod,
concat(cast(sum(Case when Churn='Yes' then 1 else 0 end)*100.0/count(*) as decimal(5,2)),'%')as churn_rate
from churn_data
...
group by Churn;
--3.Are senior citizens more likely to leave than younger customers?
SELECT 
    SeniorCitizen,
...
from churn_data
group by Contract;
---- 5.which contract type has most of the customers?
select contract,count (contract) as contract_count,
concat(cast(count(contract)*100.0/(select count(* )from churn_data) as decimal(5,2)),'%')as percentage
from churn_data
group by Contract;
--6.churn rate based on customer type
select 
Case
...
when tenure <=24 then 'medium term customer'
 ...
 --7.Does providing technical support reduce churn?"
SELECT
   TechSupport,
    COUNT(*) AS total_customers,
    SUM(CASE WHEN Churn = 'Yes' THEN 1 ELSE 0 END) AS churned_customers,
...
--8.Among churned customers, which internet service is most common?
...
--.9 Find the top 5 customers who paid the highest total charges.
