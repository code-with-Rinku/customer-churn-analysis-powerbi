...
count(gender)*100.00/(select count(*)from churn_data)as percentage
from churn_data
group by gender;
...
select  sum(TotalCharges) as total_revenue from churn_data;
...
/*select Churn,count(SeniorCitizen) as senior_citizen
from churn_data
...
concat(round(sum(case when Churn='Yes' then 1 else 0 end)*100/count(*),2),'%') as churn_rate
from churn_data
...
--For each internet service, find:
--Total customers
--Churned customers
--Average monthly charges
--Total revenue
select InternetService,count(*) as totali_customer,
...
round(avg(MonthlyCharges),2)avg_monthlycharge,
